...
do
	...
	do
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "darkmoon.club | placeholder");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"darkmoon.club" do
						Watermark.AnimateText = string.sub("darkmoon.club", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;

					for i = #"darkmoon.club" - 1, 1, -1 do
						Watermark.AnimateText = string.sub("darkmoon.club", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;
...
screen_gui.Name = "https://darkmoon.club";
...
screen_gui_2.Name = "https://darkmoon.club";
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			Name = "https://darkmoon.club",
			Parent = screen_gui_2,
			Text = '<font color="rgb(207, 227, 0)">darkmoon</font><font color="rgb(255, 255, 255)">.club</font>',
    ...
    do
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">darkmoon</font><font color="rgb(255, 255, 255)">.club</font>';
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				--// darkmoon classics
				local classics_enabled = flags["anti_aim_darkmoon_classics_enabled"];
				local classics_keybind = flags["anti_aim_darkmoon_classics_keybind"];
				local classics_types = flags["anti_aim_darkmoon_classics"];
				...
				if ((enabled or classics_enabled) and (keybind or classics_keybind) and (not (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer") or (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer"))) then
					...
					local types = {
						...
						["supercoolboi34 Destroyer"] = locals.should_darkmoon_destroy and hrp.CFrame + Vector3.new(0 / 0, 1, math.huge) or hrp.CFrame
					};
			...
			task.spawn(function()
				while task.wait(0.1) do
					locals.should_darkmoon_destroy = not locals.should_darkmoon_destroy;
				end;
			end);
...
do
	if (hookmetamethod) then
		...
		local __index; __index = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(self, property)
			if (not checkcaller() and ((((flags["anti_aim_c_sync_enabled"] or (flags["anti_aim_darkmoon_classics_enabled"] and (flags["anti_aim_darkmoon_classics"] == "Destroy Cheaters" or flags["anti_aim_darkmoon_classics"] == "supercoolboi34 Destroyer"))) and (flags["anti_aim_c_sync_keybind"] or flags["anti_aim_darkmoon_classics_keybind"]))) and property == "CFrame" and self == local_player.Character.HumanoidRootPart)) then
...
do
	...
	do
		...
		local darkmoon_classics = ui.tabs["anti_aim"]:Section({Name = "darkmoon Classics", Side = "Left", Size = 150});
		...
		--// destroy cheaters
		do
			darkmoon_classics:Toggle({Name = "Enabled", Flag = "anti_aim_darkmoon_classics_enabled"});
			darkmoon_classics:Keybind({Flag = "anti_aim_darkmoon_classics_keybind", Name = "Keybind", Default = Enum.KeyCode.Y, Mode = "Toggle"});
			darkmoon_classics:List({Name = "Classics", Flag = "anti_aim_darkmoon_classics", Options = {"Destroy Cheaters", "supercoolboi34 Destroyer"}, Default = "Destroy Cheaters"});
		end;
	end;
	...
	do --// credits to finobe wtv im way too lazy
		...
		if not isfolder("darkmoon/configs") then 
		    makefolder("darkmoon/configs");
		end;

		local function update_config_list()
		    local list = {};
		
		    for idx, file in listfiles("darkmoon/configs") do
		        local file_name = file:gsub("darkmoon/configs\\", ""):gsub(".cfg", ""):gsub("darkmoon/configs/", "");
		...
		cfgs:Button({Name = "Create", Callback = function()
		    ...
		    if config_name == "" or isfile("darkmoon/configs/" .. config_name .. ".cfg") then
		        return;
		    end;
		    writefile("darkmoon/configs/" .. config_name .. ".cfg", Library:GetConfig());
		    update_config_list();
		end});
		...
		cfgs:Button({Name = "Save", Callback = function()
		    ...
		    if selected_config then
		        writefile("darkmoon/configs/" .. selected_config .. ".cfg", Library:GetConfig());
		    end;
		end});
		...
		cfgs:Button({Name = "Load", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        Library:LoadConfig(readfile("darkmoon/configs/" .. selected_config .. ".cfg"));
		    end;
		end});
		...
		cfgs:Button({Name = "Delete", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        delfile("darkmoon/configs/" .. selected_config .. ".cfg");
		    end;
		    update_config_list();
		end});
